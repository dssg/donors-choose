...
WITH donation_totals AS 
            ...
            WHERE donations.donation_timestamp < (projects.date_posted 
                + make_interval(years => 0, 
                                ...
                                weeks => 0, 	
                                days => 0))
            GROUP BY projects.entity_id, projects.total_price_excluding_optional_support, projects.date_posted, projects.school_zip
...
WITH donation_totals AS 
        ...
        SELECT entity_id, total_donations >= total_price AS fully_funded, DATE_POSTED, school_zip
        FROM projects
	...
	JOIN funded_projects ON (projects.DATE_POSTED > (funded_projects.date_posted + make_interval(months => 4))
						 ...
						 AND projects.school_zip = funded_projects.school_zip)
