...
2. Run [main.py](main.py). This will run the Triage experiment defined in [donors-choose-config.yaml](donors-choose-config.yaml) and will generate an experiment report summary from which we can make sanity checks. Some element on the summary report: 
   * Number of temporal splits
   * Size of cohort over time
   * Base rate over time
   * Number of features in your models 
   * Model with best average performance
   * Subsets generated
   * Biases across groups
   * Models built 
   * Evaluation of the models
   * Performance of models vs bias 
   * Initial model selection based on performance
3. Run [model_selection.ipynb](model_selection.ipynb). Be sure to update `experiment_id` to match the experiment hash generated by step 1.
